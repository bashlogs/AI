...
class NQueensBestFirst {
    ...
    public static void solveNQueens(int n) {
        PriorityQueue<BoardState> openList = new PriorityQueue<>(Comparator.comparingInt(state -> state.h));
        HashSet<String> closedList = new HashSet<>();

        // Start with an empty board
        int[] initialQueens = new int[n];
        Arrays.fill(initialQueens, -1);
        BoardState initialState = new BoardState(initialQueens, heuristic(initialQueens, n));
    ...
    private static List<BoardState> generateNeighbors(BoardState state, int n) {
        ...
        for (int col = 0; col < n; col++) {
            if (isSafe(state.queens, row, col)) {
                ...
                neighbors.add(new BoardState(newQueens, h));
            }
        }
        return neighbors;
    }
